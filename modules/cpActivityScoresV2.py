...
def createDataScores(compDf: pd.DataFrame, noCompDf: pd.DataFrame = None, **kwargs):
    ...
    if noCompDf is not None:
        ...
        for (compName, currCompDf), (noCompName, currNoCompDf) in zip(compDf.groupby(level='plates'), noCompDf.groupby(level='plates')):
            acScores.append(getPlateActivityScores(compoundDf=currCompDf, noCompDf=currNoCompDf, **kwargs))
    else:
        for compName, currCompDf in compDf.groupby(level='plates'):
            acScores.append(getPlateActivityScores(compoundDf=currCompDf, noCompDf=noCompDf, **kwargs))
...
def createMultiPlot(ds, groupByCol, x, y, col_wrap=5, snsPlot=sns.scatterplot, threshold=0.5, hue=None, controlTitles = ['DMSO', 'PMA']):
    palette = None
    if hue is not None:
        palette = {f'CONTROL_{controlTitles[0]}': 'purple', f'CONTROL_{controlTitles[1]}':'limegreen','EXPERIMENTAL': 'C0'}
...
def genIndviPlots(ds: pd.DataFrame, groupByCol, xCol, yCol, outname,threshold=0.5, control=['DMSO', 'PMA']):
    ...
    with PdfPages(outname) as p:
        for name, df, in groups:
            ...
            colors = ['purple' if i== f'CONTROL_{control[0]}' else 'limegreen' if  i== f'CONTROL_{control[1]}' else 'C0' for i in df['well_type'].to_list()]
...
def genIndivElbows(ds: pd.DataFrame, groupByCol, yCol, outname, threshold=0.5, control=['DMSO','PMA'], sep='._.'):
    groups = ds.groupby(groupByCol)
...
def analyzeDf(dataset: pd.DataFrame, compLabel, outName, threshold=0.5, noCompLabel=None):
