...
def formatDf(
    ...
    **kwargs,
    ...
    compDf = (
        renameKeys(mainDf=compDf, keyDf=key, renameColumn=renameColumn)
        if key is not None
        else compDf
    )
    ...
    for control in controls:
        sepCompDf = compDf[
            compDf.index.get_level_values(renameColumn).str.contains(f"{control}{sep}")
        ]
    ...
    if noCompDf is not None:
        noCompDf = (
            renameKeys(mainDf=noCompDf, keyDf=key, renameColumn=renameColumn)
            if key is not None
            else noCompDf
        )
        ...
        for control in controls:
            sepNoCompDf = noCompDf[
                noCompDf.index.get_level_values(renameColumn).str.contains(
                    f"{control}{sep}"
                )
            ]
...
def genTreeViewClustMap(inDf: pd.DataFrame, outname, rowCluster=True, colCluster=True):
    cwd = os.getcwd()
    mkDirStr = os.path.join(cwd, ".tempDfLoc/")
    try:
        os.mkdir(mkDirStr)
    except:
        shutil.rmtree(mkDirStr)
        time.sleep(0.1)  # i need program to sleep in order to call the next command
        os.mkdir(".tempDfLoc/")

    inDf.to_csv(os.path.join(mkDirStr, "out.tsv"), sep="\t")
    pyclusterHeatmap_pearsonComplete(
        inTabFile=os.path.join(mkDirStr, "out.tsv"),
        outname=outname,
